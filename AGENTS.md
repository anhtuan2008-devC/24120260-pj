...
- Key paths: [references/paper.md](references/paper.md), [README.md](README.md), [src/App.tsx](src/App.tsx), [src/api/employees.ts](src/api/employees.ts), [src/lib/supabase.ts](src/lib/supabase.ts), [src/types/employee.ts](src/types/employee.ts), [src/utils/date.ts](src/utils/date.ts), `supabase/` (project assets), `.env` (local secrets - do not commit).
...
- Keep Supabase queries in [src/api/employees.ts](src/api/employees.ts).
- Use [src/utils/date.ts](src/utils/date.ts) for created date formatting.
- Table name fallback logic exists in [src/api/employees.ts](src/api/employees.ts) (handles `employee` and `Employee`).

Known pitfalls

- No tests are configured; verify changes manually.
- Locale formatting is hardcoded to `vi-VN` in [src/utils/date.ts](src/utils/date.ts).
- Avatar URLs depend on `VITE_AVATAR_BUCKET` and `VITE_SUPABASE_*` env vars in `.env`.
